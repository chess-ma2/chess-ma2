  ...
  while(nb_queue != 0)
  {
    ...
    //for (int c = 0; c < 3; c++) {
      ...
      for (int i = 0; i < moves->numberofmoves; i++) {
        ...
        printf("piece moved from x=%i, y=%i to x=%i, y=%i with %i from depth %i[ok]\n", currentL[c].x, currentL[c].y +1, new->x, new->y+1, new->score, new->depth);
