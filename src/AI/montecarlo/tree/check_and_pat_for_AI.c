 ...
 * @details File .c containing all the functions relating to the check, pat and checkmat for create the mcts
	      ...
	      for(int des_x = 0; des_x < 8; des_x++)
		{
		  ...
		  if( color == board2[y*8+x].color)
		      ...
		      if(isValidMove(x, y, des_x, des_y, board2) == 1)
			{
			  ...
			  board2 = pieceMove_AI(x, y, des_x, des_y, board2);


			  if (piece_to_place_AI( x_king, y_king,board2)== 0)
			    {
			      return 1; 
			    }			
...
int checkking_and_aroud_AI(int place_king_x, int place_king_y, int x_place, int y_place,struct Piece *board)
{
  ...
  board2 = pieceMove_AI(place_king_x, place_king_y, x_place, y_place, board2);
  
  for(int y = 0; y < 8; y++) 
    {
      for(int x = 0; x < 8; x++)
	{
	  if(kingcheck_place_AI(x_place, y_place, x, y, board2) == 1)
	    {
	      return 1;
	    }
	}
  ...
  for(int y = 0; y < 8; y++) 
    {
      for(int x = 0; x < 8; x++)
	{
	  if(othermove_pat2_AI(x, y, board) == 1)
	    {
	      return 1;
	    }
	}
  ...
  for(int y = 0; y < 8; y++) 
    {
      for(int x = 0; x < 8; x++)
	{
	  if(board[y*8+x].color != board[y_place*8+x_place].color && kingcheck_place_AI(x_place, y_place, x, y, board) == 1)
	    {
	      return 1;
	    }
	}
...
int checkmat_firstcondition_AI(int x_king, int y_king, struct Piece *board)
{
  ...
  if(board[(y_king)*8+(x_king-1)].color == board[y_king*8+x_king].color && board[(y_king)*8+(x_king-1)].type != 0 && (x_king - 1) <= 7 && (x_king - 1) >= 0 &&  y_king >= 0 &&  y_king <= 7)
    ...
    {
      sum += checkking_and_aroud_AI(x_king, y_king,x_king - 1, y_king, board); 
    }
  ...
  if(board[(y_king-1)*8+(x_king)].color == board[y_king*8+x_king].color && board[(y_king-1)*8+(x_king)].type != 0 && (x_king) <= 7 && (x_king) >= 0 &&  (y_king - 1) >= 0 &&  (y_king-1) <= 7)
    ...
    {
      sum +=checkking_and_aroud_AI(x_king, y_king,x_king , y_king-1, board); 
    }
  ...
  if(board[(y_king-1)*8+(x_king-1)].color == board[y_king*8+x_king].color && board[(y_king-1)*8+(x_king-1)].type != 0 && (x_king-1) <= 7 && (x_king-1) >= 0 &&  (y_king - 1) >= 0 &&  (y_king-1) <= 7)
    ...
    {
      sum += checkking_and_aroud_AI(x_king, y_king,x_king - 1, y_king-1, board); 
    }
  ...
  if ((x_king ) <= 7 && (x_king) >= 0 &&  (y_king) >= 0 &&  (y_king) <= 7)
    {
      sum += piece_to_place_AI(x_king, y_king, board);
    }
...
{
  if(checkmat_firstcondition_AI(x_king, y_king, board) == 1 && checkmat_secondcondition_AI(x_king, y_king, color, board) == 0) //valid all the condition for a check mat 
    {
      return 1;
    }
...
int pat_AI(int x_king, int y_king, struct Piece *board)
{
  ...
  if(piece_to_place_AI(x_king, y_king, board) == 0 && sum == 8 &&  othermove_pat1_AI(board) == 0) //if other movement can be make, all the place around in check and not check at the adverse king = pat 
     {
       return 1;
     }
